...
public:
    ...
    Dimension(double value) : _value{value} {}
...
// Dimension addition
template <int M, int L, int T>
Dimension<M, L, T> operator+(const Dimension<M, L, T> &lhs,
                             const Dimension<M, L, T> &rhs) {
...
Dimension<M1 + M2, L1 + L2, T1 + T2> operator*(
    const Dimension<M1, L1, T1> &lhs, const Dimension<M2, L2, T2> &rhs) {
